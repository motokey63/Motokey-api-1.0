...
- **Global scope**: `C:/motokey-api/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/motokey-api/.claude/notes/`)
...
2. Glob `C:/motokey-api/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/motokey-api/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/motokey-api/.claude/notes/` — works in any directory
