...
- **`schema.sql` — dérive non documentée restante** : au-delà des migrations 1–19 (désormais toutes reflétées), des colonnes existent en prod sur `garages`/`clients`/`interventions`/`devis` sans aucun fichier de migration correspondant (probablement des `ALTER TABLE` faits à la main via le Dashboard pendant d'autres livraisons — OR, facturation, restructuration devis). Documenté dans l'en-tête de `schema.sql`. Nécessite une recherche dédiée (type Phase 19 plan 01) avant d'être comblé. Même catégorie que la non-parité des ~19 tables déjà connues (voir `REQUIREMENTS.md` Out of Scope).
...
- [ ] SCHEMA-04 : Migrations rétroactives numérotées (20+) documentant les colonnes découvertes en Phase 20
- [ ] SCHEMA-05 : `schema.sql` mis à jour pour inclure ces colonnes
- [ ] SCHEMA-06 : Objets migration 13/15 manquants (`billing_events`, `motos_proprietaires_historique`, `liaisons_client_garage`, `reclamations_moto`, `v_motos_avec_proprietaire`) ajoutés à `schema.sql`
- [ ] SCHEMA-07 : Bootstrap vérifié propre + header known-partial-bootstrap nettoyé
...
---
*Last updated: 2026-07-09 — v1.5 Phase 20 (SCHEMA-02/03) complete — 39 colonnes cataloguées, prêt pour Phase 21*
