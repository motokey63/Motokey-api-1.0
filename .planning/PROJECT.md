...
**Target features:**
- Cycle de vie unifié : `brouillon → envoyé → accepté → en_cours → terminé → facturé` (+ `refusé`, reste modifiable, redevient éditable) — **corrigé Phase 30 (2026-07-17)** : l'audit live de l'enum `or_statut` a révélé que le set réconcilié réel est de **8 statuts**, pas 7 — `attente` (statut global on-hold, distinct du flag ligne `en_attente_acceptation_client`) et `annule` (annulation garage, distinct de `refusé` = refus client) restent tous deux conservés comme statuts à part entière ; `valide_client` devient `accepté` et `envoyé` reste un timestamp (`date_envoi`) sur un OR `brouillon`, pas un statut séparé. Détail : `.planning/phases/30-audit-sch-ma-or-statut/30-FINDINGS.md`.
...
- **v1.8 Phase 30 (2026-07-17) :** Première phase du milestone v1.8, READ-ONLY par conception. `scripts/introspect-or-statut.js` (nouveau, committé) interroge en live le spec OpenAPI PostgREST de la prod et confirme les 7 valeurs réelles de l'enum `or_statut` (`brouillon, valide_client, en_cours, attente, termine, facture, annule`) — identiques à la borne inférieure déjà prouvée par `_OR_TRANS` côté code, aucune valeur fantôme. Cross-check `pg_enum` exécuté par Mehdi au Dashboard : concordance exacte confirmée (les `enumsortorder` en `.5` prouvent que `valide_client`/`attente`/`facture` ont été ajoutés après coup via `ALTER TYPE ... ADD VALUE`, hors de toute migration trackée — la déclaration périmée à 4 valeurs de `migrations/08-livraison-3a-ordres-reparation.sql` était bien l'état d'origine). Checkpoint bloquant résolu par Mehdi avec 3 décisions produit tranchées (voir ligne "Cycle de vie unifié" ci-dessus pour le détail) qui corrigent le cadrage initial MIGR-02 : le set cible réconcilié est 8 statuts, pas 7. `.planning/phases/30-audit-sch-ma-or-statut/30-FINDINGS.md` documente la table de réconciliation complète, le plan de migration ordonné (Option A ENUM patché avec contrainte transactionnelle 55P04 vs Option B RECOMMANDÉE TEXT+CHECK, cohérente avec `devis.statut`), et sert d'entrée pour la Phase 31. Aucun DDL écrit — gate Phase 31 respecté. Vérification finale : 6/6 must-haves PASS (ré-exécution live du script pendant la vérification, byte-for-byte identique à FINDINGS.md).
...
---
*Last updated: 2026-07-17 — Milestone v1.7 (Édition devis brouillon) put ON HOLD, never shipped — scope superseded by product decision to unify Devis/OR/Facture (2026-07-16). Milestone v1.8 (Unification Devis / OR / Facture) started. Phase 30 (2026-07-17) closed the schema drift audit on `or_statut`: live introspection + Dashboard pg_enum cross-check confirmed 7 real values in prod, and Mehdi's 3 product decisions on the blocking checkpoint reconciled the target set to 8 statuses (not the 7 originally listed in MIGR-02) — see `30-FINDINGS.md`, entry point for Phase 31's DDL work. 1 known gap carried forward from v1.6 (credentials Cloudinary, since Phase 25).*
