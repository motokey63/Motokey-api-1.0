...
- **Dette RBAC transverse — `rbac.inferLegacyRole()` ne résout jamais `role='CLIENT'` (découvert v1.6 Phase 25)** : affecte 60+ endpoints dual CLIENT/GARAGE déjà en prod (interventions, devis, transfert, client device tokens, et désormais l'upload photo consommable CONSO-03) — un CLIENT authentifié via JWT legacy HS256 reçoit 403/404 au lieu du comportement attendu. Pas introduite par Phase 25 (reproduite indépendamment sur `GET /motos/:id` et `GET /devis`, déjà en prod avant cette phase). Priorité/timing d'une phase de hardening RBAC dédiée à trancher par Mehdi, idéalement avant Phase 27/28 (UI consommant ces mêmes endpoints). Détails : `deferred-items.md` (Phase 25).
...
- ✓ CONSO-03 : Upload photo consommable (côté garage validé live ; côté CLIENT bloqué par la dette RBAC transverse, voir Known Gaps) — pipeline upload Cloudinary → auto-création consommable si absente (D-05) → analyse stub → historisation — v1.6 Phase 25, 2026-07-14
...
---
*Last updated: 2026-07-14 — v1.6 Phase 25 (Endpoints Backend km/photos/remplacement compteur/Cloudinary) complete, KM-02/KM-03/CONSO-01/CONSO-03/CLOUD-01 shipped (2 items carried as known gaps: credentials Cloudinary + dette RBAC transverse)*
